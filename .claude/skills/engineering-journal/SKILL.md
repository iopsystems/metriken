...
Where an effort makes a cost/perf claim (throughput, latency, memory, query
time), the GO/NO-GO criteria and the close-out MUST carry a **measured**
number from a real run, never "bounded"/"low"/"negligible". An unmeasured
cost claim is not a GO. Flag anything you couldn't measure rather than
rounding it to "fine".
...
| "Obviously cheap — no need to measure." | Cheap is a *measured* number. "Obvious" costs are exactly where surprises hide; measure it. |
...
- Writing "bounded" / "low overhead" in an effort's GO criteria or close-out with no measured number behind it.
